...
func (h *httpTransport) Execute(ctx context.Context, transportInfo []byte, fileInfo *types.TransportFileInfo) (th transport.Handler, err error) {
	deadline, _ := ctx.Deadline()
	fileId := fileInfo.FileId
	log.Infow(fileId, "execute transfer", "file size", fileInfo.FileSize, "output file", fileInfo.OutputFile,
	...
	fi, err := os.Stat(fileInfo.OutputFile)
	if err != nil {
	...
	if fileSize > fileInfo.FileSize {
		return nil, fmt.Errorf("transfer file size=%d but file size=%d", fileInfo.FileSize, fileSize)
	}
	log.Infow(fileId, "existing file size", "file size", fileSize, "file size", fileInfo.FileSize)
	...
	if u.scheme == libp2pScheme {
		log.Infow(fileId, "libp2p-http url", "url", tInfo.URL, "peer id", u.peerID, "multiaddr", u.multiaddr)
		...
		log.Infow(fileId, "http url", "url", tInfo.URL)
	}

	// is the transfer already complete ? we check this by comparing the number of bytes
	// in the output file with the file size.
	if fileSize == fileInfo.FileSize {
		defer cleanup()

		if err := t.emitEvent(tctx, types.TransportEvent{
			NBytesReceived: fileSize,
		}, fileInfo.FileId); err != nil {
			return nil, fmt.Errorf("failed to publish transfer completion event, id: %s, err: %w", t.fileInfo.FileId, err)
		}

		log.Infow(fileId, "file size is already equal to file size, returning")
		return t, nil
	}
	...
	go func() {
		...
		if err := t.execute(tctx); err != nil {
			if err := t.emitEvent(tctx, types.TransportEvent{
				...
				log.Error(fileId, "failed to publish transport error", err)
			}
		}
	}()

	log.Infow(fileId, "started async http transfer")
	return t, nil
}
...
type transfer struct {
	...
	tInfo    *types.HttpRequest
	fileInfo *types.TransportFileInfo
	wg       sync.WaitGroup
...
func (t *transfer) emitEvent(ctx context.Context, evt types.TransportEvent, id string) error {
	select {
	case t.eventCh <- evt:
		return nil
	default:
		return fmt.Errorf("dropping event %+v as channel is full for file id %s", evt, id)
	}
}
...
func (t *transfer) execute(ctx context.Context) error {
	...
	for {
		...
		st, err := os.Stat(t.fileInfo.OutputFile)
		if err != nil {
		...
		of, err := os.OpenFile(t.fileInfo.OutputFile, os.O_APPEND|os.O_WRONLY, 0644)
		if err != nil {
		...
		remaining := t.fileInfo.FileSize - t.nBytesReceived
		reqErr := t.doHttp(ctx, req, of, remaining)
		if reqErr == nil {
			log.Infow(fileId, "http transfer completed successfully")
			// if there's no error, transfer was successful
			break
		}

		log.Infow(fileId, "http request error", "http code", reqErr.code, "outputErr", reqErr.Error())
		...
		if errors.Is(err, context.Canceled) || errors.Is(err, context.DeadlineExceeded) {
			log.Error(fileId, "terminating http transfer: context cancelled or deadline exceeded", err)
		...
		if t.nBytesReceived > st.Size() {
			log.Infow(fileId, "some data was transferred before connection error, so resetting backoff to zero",
		...
		if nAttempts >= t.maxReconnectAttempts {
			log.Errorw(fileId, "terminating http transfer: exhausted max attempts", "err", err.Error(), "maxAttempts", t.maxReconnectAttempts)
		...
		log.Infow(fileId, "backing off before retrying http request", "backoff time", duration.String(),
			"attempts", nAttempts)
		defer bt.Stop()
		select {
		case <-bt.C:
			log.Infow(fileId, "back-off complete, retrying http request", "backoff time", duration.String())
		case <-ctx.Done():
			log.Error(fileId, "did not retry http request: context cancelled", ctx.Err())
	...
	// if the number of bytes we've received is not the same as the file size, we have a failure.
	if t.nBytesReceived != t.fileInfo.FileSize {
		return fmt.Errorf("mismatch in fileSize vs received bytes, fileSize=%d, received=%d", t.fileInfo.FileSize, t.nBytesReceived)
	}
	...
	log.Infow(fileId, "http request finished successfully", "nBytesReceived", t.nBytesReceived,
		"file size", st.Size())
...
func (t *transfer) doHttp(ctx context.Context, req *http.Request, dst io.Writer, toRead int64) *httpError {
	duid := t.fileInfo.FileId
	log.Infow(duid, "sending http request", "received", t.nBytesReceived, "remaining",
	...
	for {
		if ctx.Err() != nil {
			log.Error(duid, "stopped reading http response: context canceled", ctx.Err())
		...
		if nr > 0 {
			...
			}, t.fileInfo.FileId); err != nil {
				log.Error(duid, "failed to publish transport event", err)
			}
		}
		// the http stream we're reading from has sent us an EOF, nothing to do here.
		if readErr == io.EOF {
			log.Infow(duid, "http server sent EOF", "received", t.nBytesReceived, "file-size", t.fileInfo.FileSize)
			return nil
		}
		if readErr != nil {
...
// Close shuts down the transfer for the given file. It is the caller's responsibility to call Close after it no longer needs the transfer.
